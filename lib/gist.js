...
var extractId=gist.extractId=(s)=>{
  var regexp=/[a-f0-9]{10,}/g
    ...
    if(match && match.length) {
      ...
      throw new Error("no Gist ID could be matched in string");
    }
  ...
  async post() {
    ...
    var requestOptions = {
      ...
      json: {
        ...
        public: this.public,
        //files: this.files.reduce((prev,next)=>(prev[next.title]=({content: next.content, type: next.type}) ,prev),{})  
        files: this.files
      }
    };
...
gist.fetch = function fetch(link) {
  ...
  return fetch(rawurl).then(resp=>resp.text());
}
