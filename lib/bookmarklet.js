...
function minify(code) {
  ...
  /* */
function minify(code) {
 var embed=`function xorString(s){ return [...String(s)].reduce((prev, next) => prev ^= next.charCodeAt(0), 0xFF) };
...
${code}`;
  ...
  var minified = `var script=document.querySelector('#bookmarklet')||document.createElement('script'); script.id='bookmarklet'; script.src='data:application/javascript;base64,${btoa(embed)}'; document.body.appendChild(script);`;
  return minified;
}
